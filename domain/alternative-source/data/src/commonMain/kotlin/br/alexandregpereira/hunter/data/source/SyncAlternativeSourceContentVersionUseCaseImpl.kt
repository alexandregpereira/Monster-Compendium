...
internal class SyncAlternativeSourceContentVersionUseCaseImpl(
...
) : SyncAlternativeSourceContentVersionUseCase {

    @OptIn(ExperimentalCoroutinesApi::class)
    override operator fun invoke(): Flow<SyncAlternativeSourceContentVersionUseCase.IsToSyncData> {
        return settingsRepository.getLanguage().flatMapLatest { lang ->
            flow {
                ...
                val isToSyncData = if (allChangedMap.isEmpty() && newDefaultSources.isEmpty()) {
                    SyncAlternativeSourceContentVersionUseCase.IsToSyncData(
                        isToSync = false,
